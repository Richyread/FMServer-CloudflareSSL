...
if [[ "${RESTART_SERVER:-0}" == 1 ]] ; then
    ...
    # so a timeout WARNS and continues rather than aborting a renewal that actually worked.
    server_ready() {
    ...
    sleep_interval=5                        # how often to poll
    max_wait="${MAX_WAIT_AMOUNT:-90}"       # total seconds to allow the service to return
